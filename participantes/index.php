                        ...
                        <tbody>
                            <?php while($item = mysqli_fetch_object($result)){ ?>                    
                                <tr>
                                    ...
                                    <td>
                                        <a href=<?php echo "./participant.php?id=" . $item->id; ?> class="link-secondary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">
                                            <?php echo $item->nome ?>
                                        </a>
                                    </td>
                                    <td class="d-flex flex-column gap-2">
                                        <a class='btn btn-danger' onclick="deleteParticipant(<?php echo $item->id; ?>)" style="max-width: 150px;">Excluir</a>
    ...
    <script>
        function deleteParticipant(id){
            if(confirm("Deseja realmente excluir?")){
                window.location.href = "./deleteParticipant.php?id=" + id
            }
        }
    </script>
</body>
